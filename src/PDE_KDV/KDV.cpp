...
void fdot(VecDoub_IO &u, VecDoub_IO &ut, VecDoub_IO &utt) {
  ...
  der(u, ux, uxxx);
  for (int i = 0; i < u.size(); i++)
    ut[i] = -6 * u[i] * ux[i] - uxxx[i];
  der(ut, utx, utxxx);
  for (int i = 0; i < u.size(); i++)
    utt[i] = -6 * ut[i] * ux[i] - 6 * u[i] * utx[i] - utxxx[i];
}
//----------------------------------------------------------------------------
void test_der() {
  ...
  for (int i = 0; i < NDAT; i++) {
    data[i] = 7.0 + 3.0 * cos(27 * k0 * i * dx) + 6 * sin(7 * k0 * i * dx);
  }
  for (int i = 0; i < NDAT; i++) {
    d1[i] = -3.0 * (27 * k0) * sin(27 * k0 * i * dx) +
            6 * (7 * k0) * cos(7 * k0 * i * dx);
  }
  for (int i = 0; i < NDAT; i++) {
    d3[i] = 3.0 * (27 * k0) * (27 * k0) * (27 * k0) * sin(27 * k0 * i * dx) -
            6 * (7 * k0) * (7 * k0) * (7 * k0) * cos(7 * k0 * i * dx);
  }

  der(data, d1c, d3c);

  for (int i = 0; i < data.size() / 2; i++)
    fprintf(stdout, "%8d %10.3lf %10.3lf\n", i, d1[i], d1c[i]);
  cout << "Press a key...";
  cin.get();
  for (int i = 0; i < data.size() / 2; i++)
    fprintf(stdout, "%8d %10.3lf %10.3lf\n", i, d3[i], d3c[i]);
...
void TAY() {
  ...
  fdot(u, ut, utt);
  for (int i = 0; i < u.size(); i++)
    u[i] = u[i] + ut[i] * dt + 0.5 * utt[i] * dt * dt;
}
//----------------------------------------------------------------------------
void RK2() {
...
void RK4() {
  ...
  fdot(u3, d4);
  for (int i = 0; i < u.size(); i++)
    u[i] = u[i] + (d1[i] + 2 * d2[i] + 2 * d3[i] + d4[i]) * dt / 6;
  t += dt;
}
//----------------------------------------------------------
void Leap() {
...
void ZK() { // Zabusky e Kruskal
  static bool primavolta = true, uno = true;
  static VecDoub_IO u1(NDAT), w(NDAT);
  int i;
  if (primavolta) {
    primavolta = false;
    for (int i = 0; i < u.size(); i++)
  ...
  a1[i] =
      a1[i] -
      2 * dt * (a[i + 1] + a[i] + a[NDAT - 1]) * (a[i + 1] - a[NDAT - 1]) / dx -
      dt * (a[i + 2] - 2 * a[i + 1] + 2 * a[NDAT - 1] - a[NDAT - 2]) /
          (dx * dx * dx);
  i = 1;
  a1[i] = a1[i] -
          2 * dt * (a[i + 1] + a[i] + a[i - 1]) * (a[i + 1] - a[i - 1]) / dx -
          dt * (a[i + 2] - 2 * a[i + 1] + 2 * a[i - 1] - a[NDAT - 1]) /
              (dx * dx * dx);
  for (int i = 2; i < u.size() - 2; i++) {
    a1[i] = a1[i] -
            2 * dt * (a[i + 1] + a[i] + a[i - 1]) * (a[i + 1] - a[i - 1]) / dx -
            dt * (a[i + 2] - 2 * a[i + 1] + 2 * a[i - 1] - a[i - 2]) /
                (dx * dx * dx);
  }
  i = NDAT - 2;
  a1[i] = a1[i] -
  ...
  i = NDAT - 1;
  a1[i] = a1[i] - 2 * dt * (a[0] + a[i] + a[i - 1]) * (a[0] - a[i - 1]) / dx -
          dt * (a[1] - 2 * a[0] + 2 * a[i - 1] - a[i - 2]) / (dx * dx * dx);
}
//----------------------------------------------------------------------------
double secH(double x) {
  double temp = exp(x);
  return 2.0 / (temp + 1 / temp);
}
//-----------------------------------------------------------------------------
...
void KDV() {
  static int iter = 0;
  static double sum_1 = 0, sum_2 = 0, ampli = 6.0; // 2 6 8 12
  ...
  if (primavolta) {
    ...
    for (int i = 0; i < u.size(); i++) {
      x[i] = (i - i0) * dx;
      u[i] = ampli * secH((i - i0) * dx) + (0.0 * rand()) / RAND_MAX;
    }
    filtro();
    for (int i = 0; i < u.size(); i++)
      ud[i] = 2 * u[i] / ampli;
