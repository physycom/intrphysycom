...
//-----------------------------------------------------------------------------
void Start_SingleWave() {
  // t=0; dx = 10.0/(N-1); dt= .999*dx;
  // t=0; dx = 10.0/(N-1); dt= 0.2*dx;      //RK2
  t = 0;
  dx = 10.0 / (N - 1);
  dt = 0.1 * dx; // RK2 limite in
  // t=0; dx = 10.0/(N-1); dt= .99999*dx;   //S2 stabile  SS
  // t=0; dx = 10.0/(N-1); dt= 1.00001*dx;  //S2 instabile  SS
...
//-----------------------------------------------------------------------------
void Start_WavePacket() {
  // t=0; dx = 10.0/(N-1); dt= 0.2*dx;       //S2 stabile
  t = 0;
  dx = 10.0 / (N - 1);
  dt = .99999 * dx; // S2 stabile
  // t=0; dx = 10.0/(N-1); dt= 0.2*dx;      //RK2
...
void RK2() {
  ...
  for (int n = 1; n < N - 1; n++) {
    u_[n] = u[n] + udot[n] * dt / 2;
    d_[n] = d[n] + ddot[n] * dt / 2;
  }
  u_[0] = u_[N - 2];
  u_[N - 1] = u_[1];
  d_[0] = d_[N - 2];
...
void SS() {
  ...
  for (int n = 1; n < N - 1; n++)
    u[n] = u[n] - (d[n] - d[n - 1]) * (dt / dx);
  u[0] = u[N - 2];
  u[N - 1] = u[1];
  for (int n = 1; n < N - 1; n++)
    d[n] = d[n] - (u[n + 1] - u[n]) * (dt / dx);
  d[0] = d[N - 2];
  d[N - 1] = d[1];
