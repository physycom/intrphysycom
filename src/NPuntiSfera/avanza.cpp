...
double xdot[N_PUNTI], ydot[N_PUNTI], zdot[N_PUNTI], xddot[N_PUNTI],
    yddot[N_PUNTI], zddot[N_PUNTI];
...
void Forza() {
  ...
  for (k = 0; k < N_PUNTI - 1; k++)
    for (kk = k + 1; kk < N_PUNTI; kk++) {
      r = sqrt((x[k] - x[kk]) * (x[k] - x[kk]) +
               (y[k] - y[kk]) * (y[k] - y[kk]) +
               (z[k] - z[kk]) * (z[k] - z[kk]));
      if (r < RMIN)
        r = RMIN;
...
void Distanza() {
  for (int k = 0; k < N_PUNTI; k++)
    for (int kk = 0; kk < N_PUNTI; kk++) {
      dd[k][kk] = sqrt((x[k] - x[kk]) * (x[k] - x[kk]) +
                       (y[k] - y[kk]) * (y[k] - y[kk]) +
                       (z[k] - z[kk]) * (z[k] - z[kk]));
    }
}
...
void Stringhe_Distanze() {
  ...
  for (k = 0; k < N_PUNTI - 1; k++)
    for (kk = k + 1; kk < N_PUNTI; kk++) {
      ...
      while (j < DIECI && prossimo) {
        id1 = int(MILLE * nodo[k].vicino[j].d);
        id2 = int(MILLE * nodo[kk].vicino[j].d);
        if (id2 == id1)
          j++;
        else {
          if (id2 < id1) {
  ...
  while (k0 < N_PUNTI - 1) {
    ...
    sprintf(s0, "%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf",
            ...
            nodo[k0].vicino[6].d, nodo[k0].vicino[7].d, nodo[k0].vicino[8].d,
            nodo[k0].vicino[9].d);
    while (prossimo && k1 < N_PUNTI) {
      sprintf(s1,
              "%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf%7.4lf",
              nodo[k1].vicino[0].d, nodo[k1].vicino[1].d, nodo[k1].vicino[2].d,
              nodo[k1].vicino[3].d, nodo[k1].vicino[4].d, nodo[k1].vicino[5].d,
              nodo[k1].vicino[6].d, nodo[k1].vicino[7].d, nodo[k1].vicino[8].d,
              nodo[k1].vicino[9].d);
      if (strcmp(s1, s0) == 0) {
        k_tot++;
        k0++;
